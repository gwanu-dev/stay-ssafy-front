...
const BASE_LIKE_URL = import.meta.env.VITE_BASE_URL + "/like";
...
export const useLikeAxiosStore = defineStore(
    ...
    () => {
        ...
        const confirmLike = async (articleNo, memberId) => {
            let res = await local.get(BASE_LIKE_URL + `?articleNo=${articleNo}&memberId=${memberId}`);
            console.log(CONSOLE_ROUTE, "confirm like : ", res);
            return res;
        }
